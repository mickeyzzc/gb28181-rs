...
- **Media push** — H.264 NALUs → MPEG-2 Program Stream → RTP (UDP + RTP-over-TCP framed), SSRC handling
...
Not included (by design): platform/UAS role, SIP over TLS/WebSocket, H.265 media (host-side extension point).
...
v0.1.0 — API surfaces (`FrameSource`, `RecordingSource`, config) are settling but not yet frozen. Production-tested daily at [Mi-Bee Studio](https://github.com/Mi-Bee-Studio) against the MiBee NVR GB28181 platform.
