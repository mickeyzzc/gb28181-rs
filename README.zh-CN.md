...
- **DeviceControl 子命令**（2016 §9.3.2 / 2022 §9.3）—— `IFrameCmd` 强制 I 帧（平台起流/丢包恢复时常发）、`RecordCmd`（Record/StopRecord）、`GuardCmd`（SetGuard/ResetGuard）、`AlarmCmd`（ResetAlarm）、`TeleBoot`（Boot）与 `PTZCmd`（位级解码：§A.3-A.4 运动方向/速度位、预置位、巡航、FI 聚焦/光圈、辅助开关——`parse_ptz_command`，gb28181-go platform 构造器的逐字节逆运算，golden 十六进制表跨孪生共享；无法解码的输入以 `PtzCommand::Invalid` 原样保留）从同一 Control 体解码，经 `DeviceControlHandler` 宿主接缝回调（默认全 no-op，按硬件能力按需实现）；未识别子命令保持合规的 control reject。UDP 传输；DragZoom 待核验 2022 线格式后再解码
- **DeviceConfig 最小 + ConfigDownload**（2022 §9.3.3 / A.2.3.2、A.2.4.7）—— 可落地子集带宿主回调（`DeviceConfigHandler`）：`BasicParam`（名称+注册参数——宿主自行决定生效范围）、`FrameMirror`（0-3 翻转模式）、`AlarmReport`（移动侦测/区域入侵上报开关）；handler 接受的命令答 `Result=OK`，其余保持既有 reject。ConfigDownload 查询按请求回最小合规 Response，请求含 BasicParam 时带上该块（取自运行配置的名称/有效期/心跳参数）。注意：2022 校时不是 DeviceConfig 子命令——§9.10.2 走 REGISTER 应答的 SIP Date 头。`HomePosition` 并入 DeviceControl 家族（A.2.3.1.10：Enabled/ResetTime/PresetIndex → `on_home_position`）
- **语音广播（接收侧，§9.12.1）** —— Broadcast 通知（A.2.5.5）先应答（A.2.6.11，装了音频 sink 才答 OK），随后设备向广播源回打 audio-only INVITE（s=Play / m=audio / `<source>:<ssrc>,<device>:0` Subject，Request-URI 指平台实际地址），200 后 ACK 并经共享媒体槽接收平台 G.711 RTP——BYE 正常拆除。会话单槽、INVITE 5s 应答窗口、无 sink 时以 Result=ERROR 婉拒
...
- **回放/下载完成通知** —— 会话自然结束时发送会话内 §9.4.2 `MediaStatus: Play Finished` / `Download Finished` SIP INFO（MANSRTSP 风格消息体），平台可立即收流而无需等待停滞超时；BYE 侧拆除保持静默。UDP 传输
...
`Gb28181Config` 对 serde 友好（TOML/JSON），可直接重导出到宿主自己的配置结构体。地址类默认值沿用国标示例值（`platform_sip_address = 192.168.1.1`、`device_id = 34020000001320000001`、端口 5060）—— **生产环境务必显式设置**；示例默认值仍生效时，服务端启动会输出警告日志。SIP 密码**没有默认值**：初始为空，未设置或仍是广为人知的示例值 `12345678` 时服务端会告警。
